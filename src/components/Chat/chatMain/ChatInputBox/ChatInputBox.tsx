    ...
    const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file) {
            ...
            reader.onload = () => {
                ...
                contentRef.current?.insertAdjacentHTML('beforeend', `<img src="${base64}" className="max-w-5rem inline-block align-middle" alt="Uploaded Image" />`);
                contentRef.current?.focus();
            }
        }
    }
        ...
        <div className="chatInputBoxContainer h-auto w-full">
            <div className={"w-full bg-sky-400 h-10 flex items-center "}>
                <div className="relative inline-block emojiBox ">
                    <Button className={"bg-transparent h-full  text-xl"} onClick={handleEmojiPickerToggle}>😊</Button>
                    {showEmojiPicker && (
                ...
                <div className="relative inline-block">
                    <input type="file" accept="image/*" onChange={handleImageUpload} className="hidden"
                           ref={imageUploadRef}/>
                    <Button className={"bg-transparent h-full  text-xl"}
                            onClick={() => imageUploadRef.current?.click()}>📷</Button>
                </div>
                <div>
                </div>
            </div>
            <div className={"chatsInputBox"}>
                <div
                    ...
                    contentEditable
                    style={{border: '1px solid #ccc', minHeight: '10rem', padding: '8px'}}
                ...
                />
                <Button type="primary" onClick={handleSendMessage} className="chatSendBtn">Send</Button>
            </div>
        </div>
    )
}
