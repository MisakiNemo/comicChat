...
export type ContentType = TextContent | ImageContent | VideoContent | VoiceContent
...
const ChatStore = createSlice(
    {
        ...
        reducers: {
            ...
            removeMessage: (state, action: PayloadAction<Message>) => {
                const chat = state.chats.find(chat => chat.ChatId === action.payload.chatId)
                if (chat) {
                    chat.messages = chat.messages.filter(message => message.messageId !== action.payload.messageId)
                }
            },
            updateMessage: (state, action: PayloadAction<{ chatId: string, message: Message }>) => {
                const chat = state.chats.find(chat => chat.ChatId === action.payload.chatId)
                if (chat) {
                    const messageIndex = chat.messages.findIndex(message => message.messageId === action.payload.message.messageId)
                    if (messageIndex !== -1) {
...
export const { addChat, removeChat, addMessage, removeMessage, updateMessage } = ChatStore.actions
